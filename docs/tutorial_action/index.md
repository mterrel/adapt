...
- `context` is an ActionContext that contains information and objects that can be useful for a component, such as a logger for logging messages.
...
It then checks the value of `diff` -- if `diff` is `"delete"` then the deployment is requesting that the file should **not** exist.
...
The else clase again checks whether the file exists and compares the contents to create a message in `detail` that describes the action that will be taken, or in the fall-through case, the contents are correct, so `detail` is not set.

If no action needs to be taken (`detail` is not set), `shouldAct` returns `false`.
Otherwise, a `ShouldAct` object is returned that says action is required (`act: true`) and gives `detail` as the description of the action.
...
Experiment with changing the `contents` prop in `index.tsx` or adding another `<LocalFile>` element in `App` and running `adapt update test` to make the changes take effect.
...
In AWS, almost every resource you can create has an ID called an ARN.
